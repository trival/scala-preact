...
/** Base trait for HTML element companions. Provides modifier-based apply method
  * and prop modifiers.
  *
...
trait ElementCompanion[T]:
  /** Tag name for this element (e.g., "div", "span", "button") Must be
    * implemented by concrete companions
    */
  def tagName: String

  /** Build element from case class instance Must be implemented to call h()
    * with proper props
    */
  def buildVNode(instance: T): VNode

  /** Apply method that accepts modifiers This is what makes div(...) syntax
    * work
    */
...
/** Simplified element companion for elements with only children
  */
abstract class SimpleElementCompanion[T](val tagName: String)
    extends ElementCompanion[T]:
  def buildVNode(instance: T): VNode =
...
object PropModifiers:
  ...
  inline def disabled(value: Boolean): Modifier =
    PropModifier("disabled", value)
