...
/** Context that provides methods for creating reactive Vars.
  * Different implementations provide global (signal) or component-local
  * (useSignal) variants.
  */
trait VarContext:
...
object GlobalVarContext extends VarContext:
  def createVar[A](initialValue: A): Var[A] =
    println(s"[GlobalVarContext] Creating signal with initial value: $initialValue")
...
object ComponentVarContext extends VarContext:
  def createVar[A](initialValue: A): Var[A] =
    println(s"[ComponentVarContext] Creating useSignal with initial value: $initialValue")
